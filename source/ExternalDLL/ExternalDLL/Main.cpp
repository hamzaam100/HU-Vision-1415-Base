...
int main(int argc, char * argv[]) {
	...
	std::vector<std::string> names{ { "female-1", "female-2", "female-3", "male-1", "male-2", "male-3", "child-1" } };

	ImageFactory::setImplementation(ImageFactory::DEFAULT);

	ImageIO::debugFolder = parentFolderPath + "debug";
	ImageIO::isInDebugMode = true; //If set to false the ImageIO class will skip any image save function calls
	RGBImage * input = ImageFactory::newRGBImage();
	if (!ImageIO::loadImage(parentFolderPath + "testsets\\Set A\\TestSet Images\\" + names[1] + ".png", *input)) {
...
bool executeSteps(DLLExecution * executor) {
	...
	if (!executor->executePreProcessingStep1(true)) { //stepToIntensityImage
		std::cout << "Pre-processing step 1 failed!" << std::endl;
		return false;
	}

	if (!executor->executePreProcessingStep2(true)) { //stepScaleImage
	...
	if (!executor->executePreProcessingStep3(true)) { //stepEdgeDetection
	...
	if (!executor->executePreProcessingStep4(true)) { //stepThresholding
	...
	if (!executor->executeLocalizationStep5(true)) { //stepFindExactEyes
	...
	if (!executor->executeExtractionStep1(true)) { //stepExtractEyes
